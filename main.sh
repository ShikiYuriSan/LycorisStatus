...
fb_page="https://www.facebook.com/lycoframe"
counter_like=0
counter_followers=0
inc_frame=0
...
until [[ "${counter_like}" -eq "${status_like}" ]] && [[ "${counter_followers}" -eq "${status_followers}" ]]; do
	increment_val="$(shuf -i 10-40 -n 1)"
	increment_val2="$(shuf -i 10-40 -n 1)"
	...
	convert -size 1280x480 xc:"#ffffff" \
		...
		-pointsize 20 -annotate +102+295 "${page_name}" \
		-fill "#ffffff" -annotate +103+194 "${page_name}" \
